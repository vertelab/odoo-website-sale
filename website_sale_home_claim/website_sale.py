...
class website_sale_home(website_sale_home):
    ...
    def home_page_claim(self, home_user=None, claim=None, **post):
        ...
        return request.render('website_sale_home_claim.page_claim', {
    ...
    @http.route(['/home/<model("res.users"):home_user>/order/<model("sale.order"):order>/claim','/home/<model("res.users"):home_user>/line/<model("sale.order.line"):line>/claim'], type='http', auth="user", website=True)
    def home_page_claim_order(self, home_user=None, order=None,line=None, **post):
        if order:
            claim = request.env['crm.claim'].sudo().create({'name': order.name, 'partner_id': order.partner_id.id, 
                       'partner_phone': order.partner_id.phone, 'email_from': order.partner_id.email,
                       'ref': "sale.order,%s" % order.id,
                       }) 
        elif line:
            claim = request.env['crm.claim'].sudo().create({'name': line.product_id.name, 'partner_id': line.order_id.partner_id.id, 
                       'partner_phone': line.order_id.partner_id.phone, 'email_from': line.order_id.partner_id.email,
